...
public class InteractableWater : MonoBehaviour
{
    ...
    private Coroutine _continuousSplashCoroutine;
    ...
    public void GenerateMesh()
    {
        ...
        for (int y = 0; y < NUM_OF_Y_VERTICES - 1; y++)
        {
            ...
            for (int x = 0; x < numOfXVertices - 1; x++)
            {
                ...
                int topLeft = bottomleft + numOfXVertices; int topRight = topLeft + 1;
    ...
    public void StartContinuousSplash(Vector2 position, float force, float interval = 0.1f)
    {
        if (_continuousSplashCoroutine == null)
        {
            _continuousSplashCoroutine = StartCoroutine(ContinuousSplash(position, force, interval));
        }
    }

    private IEnumerator ContinuousSplash(Vector2 position, float force, float interval)
    {
        while (true) // Bucle infinito hasta que se detenga manualmente
        {
            PerformSplash(position, force);
            yield return new WaitForSeconds(interval);
        }
    }

    public void StopContinuousSplash()
    {
        if (_continuousSplashCoroutine != null)
        {
            StopCoroutine(_continuousSplashCoroutine);
            _continuousSplashCoroutine = null;
        }
    }

    private void PerformSplash(Vector2 position, float force)
    {
        for (int i = 0; i < _waterPoints.Count; i++)
        {
            Vector2 vertexWorldPos = transform.TransformPoint(_vertices[_topVerticesIndex[i]]);

            if (Vector2.Distance(vertexWorldPos, position) < 0.5f) // Ajusta este valor según la precisión deseada
            {
                _waterPoints[i].velocity = force;
            }
        }
    ...
    public void SplashAt(Vector2 position, float force)
    {
        float radius = 0.5f; // Puedes ajustar este valor según lo necesites

        for (int i = 0; i < _waterPoints.Count; i++)
        {
            Vector2 vertexWorldPos = transform.TransformPoint(_vertices[_topVerticesIndex[i]]);

            if (IsPointInsideCircle(vertexWorldPos, position, radius))
            {
                _waterPoints[i].velocity = force;
            }
        }
...
public class InteractableWaterEditor : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        Vector3 size = new Vector3(_water.width, _water.height, 0.1f); Handles.DrawWireCube(center, size);
